...
public class ProductController {
   ...
   @GetMapping("/{id}")
    ...
    public ResponseEntity<List<Product>> searchProducts(@RequestBody ProductRequest productRequest) {
        List<Product> foundProducts = productService.searchProducts(productRequest);
        return ResponseEntity.ok(foundProducts);
    }
    @PostMapping("/search2")
    public ResponseEntity<List<Product>> searchProducts2(@RequestBody ProductRequest productRequest) {
        List<Product> foundProducts = productService.searchProducts(productRequest);
        return ResponseEntity.ok(foundProducts);
    }
}
